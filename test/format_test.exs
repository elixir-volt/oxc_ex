defmodule OXC.FormatTest do
  ...
  describe "format/3" do
    test "formats messy JavaScript" do
      {:ok, code} = OXC.Format.run("const   x=1;let   y =  2;", "test.js")
    ...
    test "single_attribute_per_line forces one attribute per line" do
      ...
      {:ok, multi} =
        OXC.Format.run(input, "test.jsx", single_attribute_per_line: true, print_width: 80)

      {:ok, auto} =
        OXC.Format.run(input, "test.jsx", single_attribute_per_line: false, print_width: 80)
    ...
    test "embedded_language_formatting: :off disables embedded formatting" do
      {:ok, code} =
        OXC.Format.run("const x = 1", "test.js", embedded_language_formatting: :off)
  ...
  describe "format!/3" do
    test "returns formatted code directly" do
      code = OXC.Format.run!("const   x=1", "test.js")
      assert code == "const x = 1;\n"
    end

    test "raises on error" do
      assert_raise OXC.Error, fn ->
        OXC.Format.run!("const = ;", "test.js")
      end
    end
  end
end
