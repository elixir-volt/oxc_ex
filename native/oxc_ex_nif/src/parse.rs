...
fn json_value_to_term<'a>(env: Env<'a>, value: Value) -> Result<Term<'a>, String> {
    match value {
        ...
        Value::Number(value) => {
            if let Some(value) = value.as_i64() {
                ...
                Err(format!("invalid JSON number: {value}"))
            }
        }
        Value::String(value) => Ok(value.encode(env)),
        Value::Array(values) => values
            .into_iter()
            .map(|value| json_value_to_term(env, value))
        ...
        Value::Object(values) => {
            ...
            for (key, value) in values {
                map = map
                    .map_put(key, json_value_to_term(env, value)?)
...
fn json_str_to_term<'a>(env: Env<'a>, json: &str) -> Result<Term<'a>, String> {
    ...
    Value::deserialize(&mut deserializer)
        ...
        .and_then(|value| json_value_to_term(env, value))
}
